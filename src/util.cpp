...
const char * const BITCOIN_CONF_FILENAME = "cannabcoin.conf";
const char * const BITCOIN_PID_FILENAME = "cannabcoind.pid";
...
#else
    const char* pszModule = "cannabcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cannabcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cannabcoin";
#else
    // Unix
    return pathRet / ".cannabcoin";
#endif
#endif
}
